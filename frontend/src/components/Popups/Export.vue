<template>
	<div class="export">
		...
		<div class="buttons">
			<FormKit type="button" label="JSON" button-type="secondary" @click="exportLinks('json')" />
			<FormKit type="button" label="CSV" button-type="primary" @click="exportLinks('csv')" />
		</div>
	</div>
</template>
...
export default {
	...
	methods: {
		async exportLinks(format) {
			...
			if (response.success) {
				const buffer = new Uint8Array(response.result.buffer.data).buffer;
